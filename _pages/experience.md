---
permalink: /experience/
title: ""
author_profile: true
---

<!-- This page lists all my professional, extracurricular and volunteering experiences. -->

Research Experience
======

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Undergraduate Research Assistant & Co-Lead (AI/ML Systems)</b></span>
  <span><i>May 2025 - Present</i></span>
</div>
<p>KineTex Lab (KIIT Chapter), KIIT University, Bhubaneswar, India <a href="https://www.kinetex.in/">Visit</a></p>
------

**Key Responsibilities:**
- Conduct AI/ML-driven biomedical research in medical imaging, multimodal learning, and XAI; develop deep learning
and transformer models using large-scale clinical datasets, collaborate with PhD scholars on ongoing projects, contribute
to publications, and mentor undergraduate researchers through structured research guidance.



<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Co-founder and Researcher</b></span>
  <span><i>May 2025 - Present </i></span>
</div>
<p>Biofolk (Remote) <a href="https://biofolk.info/">Visit</a></p>
------

**Key Responsibilities:**
- Co-founded BioFOLK, an interdisciplinary research and knowledge-exchange platform, contributing to its vision and
research direction, leading collaborative initiatives, and fostering interdisciplinary research culture, education, and global collaboration.



Other Experience
======

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Co-Founder</b></span>
  <span><i>Jul 2021 - Present</i></span>
</div>
<p>Techsum Production <a href="https://www.facebook.com/techsum.production">Visit</a></p>
------

**Key Responsibilities:**
- As Co-Founder, I scaled a premier production house, delivering end-to-end broadcast and technical solutions for high-profile corporate clients, national television channels, and government events. My role was comprehensive, involving leading all client-facing activities—from initial meetings and requirements gathering through to final project execution—to ensure perfect alignment with client objectives and timelines. Concurrently, I served as the lead Broadcast Engineer and on-site Team Lead for all sound and video operations, personally managing technical crews to guarantee flawless live event production. This integrated approach was key to successfully delivering technical production for nationally significant events, including the historic Padma Bridge opening ceremony, live sports broadcasts for the Bangladesh national football and cricket teams, and major commercial campaigns such as the Daraz 11.11 sale. We also provided critical live broadcast feeds for leading television channels like BTV, ETV, and Desh TV. Since relocating to India in 2023 to pursue my undergraduate degree in Computer Science at KIIT University, I have transitioned to a fully remote leadership role. I continue to oversee all company operations and client projects while serving as the primary technical expert, guiding my team remotely to solve any on-the-ground challenges and ensure flawless execution.



<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Head of ICT Cell</b></span>
  <span><i>Mar 2020 - May 2023 </i></span>
</div>
<p>Head of ICT Cell Sheikh Russel National Children and Adolescent Council</a></p>
------

**Key Responsibilities:**
- As the Head of the ICT cell, I led the organization's digital transformation, significantly enhancing its online presence and outreach. My role involved managing the IT team, website, social media, and broadcasting department. A key highlight was successfully managing live broadcast feeds for national television channels during high-profile events attended by the Prime Minister of Bangladesh and other government ministers.

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>IT & MIS(Internship</b></span>
  <span><i>Feb 2021 - Dec 2023 </i></span>
</div>
<p>TAKAS ENTERPRISE</a></p>
------

**Key Responsibilities:**
- As an IT & MIS Intern at TAKAS ENTERPRISE (Feb 2021 - Dec 2023), I was responsible for developing and managing the company's ERP system and its complete IT infrastructure, including server administration and network support. My role also extended to overseeing the company's digital footprint through website management, active social media management, and executing broader digital marketing campaigns. I also contributed to the maintenance of the company's Management Information System (MIS).

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Web Developer</b></span>
  <span><i>Feb 2021 - Dec 2023 </i></span>
</div>
<p>Freelance</a></p>
------

**Key Responsibilities:**
- Developed WordPress websites and provided PHP MySQL ERP solutions for schools and small businesses. Ensured clients had functional, easy-to-manage websites, contributing to business operations and growth.

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Technical Support, ISP</b></span>
  <span><i>Mar 2017 - Dec 2019 </i></span>
</div>
<p>A. M Network [kalapara]</a></p>
------

**Key Responsibilities:**
- Worked with MikroTik, Cisco, and fiber optic cables to provide internet services and troubleshooting support to clients.Provided solutions for network-related issues and ensured high levels of customer satisfaction.

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Service TechnicianP</b></span>
  <span><i>Jan 2016 - Dec 2019 </i></span>
</div>
<p> A.M Computer Service</a></p>
------

**Key Responsibilities:**
- My journey into technology began with a promise from my father. Growing up near the sea beach of Kuakata, in an undeveloped area, he told me that if I did well on my PSC exam, he would buy me a laptop. True to his word, when I entered high school, I received my first computer. At that time, computers were rare in my village you might see one at a police station, government office, or a shop for Skype calls and emails. Most people, even computer owners, had little idea how to fix them. For serious issues, machines had to be sent to Barishal or even Dhaka.
- My steep learning curve began when I tried to install a game and was blocked by a graphics driver issue. No one could help, so I turned to my father’s Nokia C2 phone, tethered by USB for a painfully slow internet connection. That was my breakthrough. I learned what a driver was, found the right one, and solved the problem myself. This success ignited my passion. I went deeper exploring disk partitioning, MBR vs GPT, UEFI vs legacy boot, and troubleshooting hardware and software. I learned to install Windows (XP, 7, 8.1, 10) and Linux, and to diagnose issues with RAM, hard disks, motherboards, and network cards.
- Word spread that a young student knew computers. The owner of a newly opened service center contacted me. His model was simple: collect faulty machines and send them to the city for repair. Instead, he gave me a chance. I used research to fix problems locally. Once, a “black screen” PC came in. I suspected faulty RAM, swapped it with another, and it worked instantly. I had proven myself.
- While repairing machines, I saw a bigger issue our community’s isolation came from the lack of reliable internet. I proposed to the service center owner: “Instead of just fixing computers, why don’t we become an ISP and bring the internet to our town?” He believed in my vision. Together, we secured a license, bought bandwidth, and with one MikroTik router and Cat 6 cables, began providing connections house by house.


Education
======

<div style="display: flex; justify-content: space-between;">
  <span style="font-size:20px"><b>Bachelor of Technology in Computer Science and Engineering (CSE)
</b></span>
  <span><i>Jun 2023 - Present</i></span>
</div>
<p>Kalinga Institute of Industrial Technology, Bhubaneswar, India</p>
------

